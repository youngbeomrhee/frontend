 ...
 *
 * com/davidflanagan/Shapes.js : 도형을 표현하는 클래스들의 모듈
 *
 * 이 모듈은 네임스페이스 com.davidflanagan.shapes 안에서 클래스를 정의한다.
...
// 우리는 Class 모듈을 검사하면서 네임스페이스 com.davidflanagan이 존재한다는 사실을
// 알았기 때문에 여기서 따로 생성하지 않아도 된다.
// 따라서, 그냥 이 네임스페이스에서 도형들을 생성한다.
if(com.davidflanagan.shapes) {
    throw new Error("com.davidflanagan.shapes namespace alread exists");
}
...
com.davidflanagan.shapes.Circle = define({/* 클래스 데이터 */});
com.davidflanagan.shapes.Rectangle = define({/* 클래스 데이터 */});
com.davidflanagan.shapes.Triangle = define({/* 클래스 데이터 */});