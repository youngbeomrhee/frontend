...
function solution(participants, completions) {
    ...
    for (let i = 0, len = participants.length; i < len; i++) {
        ...
        if(!participantsHash[participant]) {
            participantsHash[participant] = 1;
        } else {
            participantsHash[participant] = participantsHash[participant] + 1;
        }
    }

    for (let i = 0, len = completions.length; i < len; i++) {
        const participant = completions[i];
        const count = participantsHash[participant];
        if(count === 1) {
            delete participantsHash[participant];
        } else {
            participantsHash[participant] = count - 1;
        }
    }

    return Object.keys(participantsHash)[0];
}
...
console.assert(solution(["marina", "josipa", "nikola", "vinko", "filipa"], ["josipa", "filipa", "marina", "nikola"]) === 'vinko');
console.assert(solution(["leo", "kiki", "eden"], ["eden", "kiki"]) === 'leo');
