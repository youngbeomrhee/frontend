...
function solution(participants, completions) {
    const completionsHash = completions.reduce((hash, p) => (hash[p] = hash[p] ? hash[p] + 1: 1, hash), {});
    return participants.find(p => (completionsHash[p] && completionsHash[p] > 0 ? (completionsHash[p] = completionsHash[p] - 1, false) : true));
}
