...
// 이런 구조로 객체를 설계할 경우 메서드 체인을 사용할 수 있다.
log(`createPerson()
        .setFirstName('Mike')
        .setLastName('Fogus')
        .setAge(108)
...
_.chain([]).constructor === _([]).constructor;

/*------------------------------- underscore의 chain 코드 종료 */


/* chain의 동작을 흉내내보기 */
var myChain = function (arr) {
    function F(paramArr) {
        this.arr = paramArr;
    }
    F.prototype = {
        sort: function() { this.arr = this.arr.sort((a,b)=>a-b); return this; },
        tap: function() { console.log(this.arr); return this; },
        value: function() { return this.arr; }
    }
    return new F(arr);
};

// 실행
myChain([5,2,6,11,2,4,25,2,1])
    .tap()
    .sort()
    .tap()
    .value();
...
var TITLE_KEY = 'titel';    // 오타로 잘못된 값이 들어간 경우
...
// _.tap : Invokes interceptor with the object, and then returns object. The primary purpose of this method is to "tap into" a method chain, in order to perform operations on intermediate results within the chain.
...
log("rev(_.rest(_.initial(_.compact([2, 3, null, 1, 42, false])))) -> "
